                                        ...
                                        <div class="col-md-2">
                                            <div class="card">
                                                <div class="card-img rounded-circle">
                                                    ...
                                                    <img src="	https://gifdb.com/images/high/2d-gengar-grinning-pokemon-upw3s7evy3v921cb.gif"
                                                        class="img-fluid" >
                                                    </a>
                                                </div>
                                                <div class="card-img-overlays text-warning" style="font-weight: bold;  text-transform: uppercase; font-size: 24px;">${element.name}</div>
                                            </div>
                                        </div>
                                    </div>`;
      } else {
...
function mostrarCategorias() {
  getTypePokemon().then((response) => {
    let categoriass = `<div class="btn-group" role="group">
                        <button id="btnGroupDrop1" type="button" class="btn btn-danger btn-lg dropdown-toggle"
...
function autoCompletePokemon() {
  ...
  if (textoBuscar.length >= 3) {
    ...
    filtroPokemon.forEach((element) => {
      ...
      lista += `<a onclick="detallePokemon('${element.url}')" href="pokemonDetalle.php" class="list-group-item list-group-item-action">${element.name} <img id="icono${element.name}"></a>`;
    });
    lista += "</div>";
    document.getElementById("listPokemon").style =
      "overflow:scroll;height:300px;width:25%;position:absolute;z-index: 1;";
...
function carritoPokemon() {
  ...
  carritoCompras.forEach((element, index) => {
    items += `
            <div class="row my-3">
                ...
                <div class="col-6">
                        <div class="row">Nombre:-ID: ${element._id}</div>
                        <div class="row">Precio: ${element._precio} x ${
      element._cantidad
    } = ${element._precio * element._cantidad}</div>
                        <div class="row">Cantidad: <input onclick="actCantidad(this,${index})" onkeyup="actCantidad(this,${index})" class="form-control" type="number" value="${
      element._cantidad
    }"></div>
                        <div class="col-2">
  ...
  document.getElementById(
    "cantidadItems"
  ).innerHTML = `${carritoCompras.length}+`;
  totalCompra();
}
...
function totalCompra() {
  ...
  document.getElementById(
    "totalPagar"
  ).innerHTML = `<b>Total: $ </b> ${totalPagar}`;
}
